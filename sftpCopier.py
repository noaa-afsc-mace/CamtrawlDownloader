...
class sftpCopier(QtCore.QObject):
    ...
    def __init__(self, name, parent, verify=False,
            matchTimes=False, dataRoot='/camtrawl/image_data',
            hostKeys='./.known_hosts'):

        #  initialize the base class
        super(sftpCopier, self).__init__(parent)
        ...
        self.sshClient.load_host_keys(hostKeys)
    ...
    def connect(self, host, user, password):
        ...
        self.sshClient.connect(host, username=user, password=password)
        self.sftp = paramiko.SFTPClient.from_transport(self.sshClient.get_transport())
        self.sftp.chdir(self.dataRoot)
    ...
    def getDeployments(self):
        ...
        for d in deployments:
            remotePath = d + '/' + 'logs/CamtrawlMetadata.db3'
            localPath = self.tempFolder + '/CamtrawlMetadata.db3'
            self.sftp.get(remotePath, localPath)
            self.getDeploymentDetails()



    def getDeploymentDetails(self, deployments):
